...
# Refactorización: Reparación de métodos
...
class Zombie:
    ...
    def ultimo_cerebro(self) -> datetime:
        ''' Retorna la hora del último cerebro consumido '''
    ...
    def info(self):
        ...
        Último cerebro: {}'''.format(
            self.__nombre,
            self.__tipo.name,
            self.ultimo_cerebro() # Extract method aplicado
            #Previamente: datetime.now() - self.__ultimo_alimento
        )
    ...
    def estado_de_animo(self):
        ...
        # Se aplicó inline method para todos los siguientes if's
        # if self.mas_de_10_segundos_sin_comer():
        if self.ultimo_cerebro() > timedelta(self.ENFADADO):
            return 'agresivo' # Inline variable aplicado
            # Previamente se definia variable de estado con valor 'agresivo'

        # elif self.mas_de_20_segundos_sin_comer():
        if self.ultimo_cerebro() > timedelta(self.AGRESIVO):
            return 'enfadado'

        # elif self.mas_de_5_segundos_sin_comer():
        elif self.ultimo_cerebro() > timedelta(self.NORMAL):
            return 'normal'
        else:
            return 'contento'
    ...
    def peligrosidad(self):
        ...
        # Se aplicó extract variable, previamente se tenían if's similares con cambios mínimos
        tipo_agresivo = self.__tipo == TipoZombie.ZOMBIEINSTEIN or self.__tipo == TipoZombie.ZOMBIEPULTA
        buen_animo = self.estado_de_animo == 'contento' or self.estado_de_animo == 'normal'
        cerebros_qty = self.__cerebros_devorados >= 1

        if not tipo_agresivo and buen_animo and cerebros_qty:
            return 'baja'
        elif not tipo_agresivo and not buen_animo and cerebros_qty:
            return 'media'
        else:
            return 'alta'
    ...
    # Se aplicó replace temp with query
    def tiempo_de_vida_restante(self) -> datetime:
        ''' Regresa el tiempo de vida restante del zombie '''
        return self.tiempo_de_vida_total - (datetime.now() - self.__timestamp)
    ...
    def buscar_cerebros(self, radio: int):
        ...
        # Se argumenta que en este caso no sería necesario declarar variables adicionales
        # ya que los cálculos se realizan una sola vez dentro del mismo string, quedando
        # anteriormente como se muestra en las siguientes líneas
        # return f'''Detalles de la búsqueda
        # perímetro: {2 * pi * radio} km
        # area     : {pi * pow(radio, 2)} km
        # '''

        # Se aplicó split temporary variable
